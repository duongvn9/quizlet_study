...
const response = z.object({ selectedOptionIds: selection.optional(), selectedOptionId: z.string().min(1).optional(), answeredAt: z.string() }).transform(({ selectedOptionId, ...value }) => ({ ...value, selectedOptionIds: value.selectedOptionIds ?? [selectedOptionId!] }));
