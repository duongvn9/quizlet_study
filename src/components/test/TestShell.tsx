...
export function TestShell({ subject }: { subject: Subject }) {
  ...
  const submit = useCallback(() => { if (!session) return; const unanswered = session.questionIds.filter((id) => !session.responses[id]).length; if (confirm(`Bạn còn ${unanswered} câu chưa trả lời. Nộp bài ngay?`)) commit(submitTest(session, subject.questions, dependencies.now())); }, [commit, session, subject.questions]);
