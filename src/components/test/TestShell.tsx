...
export function TestShell({ subject }: { subject: Subject }) {
  ...
  const start = useCallback((settings: TestSettings) => { if (session?.status === "active" && !confirm("Bài kiểm tra chưa hoàn thành sẽ bị thay thế. Bạn có muốn tạo bài mới?")) return; const pool = eligibleQuestions(subject.questions, settings.pool, learn?.questionProgress); commit(createTestFromPool(subject.id, subject.contentVersion, pool, settings, dependencies)); setNotice(null); }, [commit, learn, session, subject]);
  const submit = useCallback(() => { if (!session) return; const unanswered = session.questionIds.filter((id) => !session.responses[id]).length; if (unanswered === 0 || confirm(`Bạn còn ${unanswered} câu chưa trả lời. Nộp bài ngay?`)) commit(submitTest(session, subject.questions, dependencies.now())); }, [commit, session, subject.questions]);
  ...
  return <>{notice && <div className="notice" role="status">{notice}</div>}{!session ? <TestSetup total={subject.questions.length} unmastered={unmastered} onStart={start} /> : session.status === "active" ? <TestRunner subject={subject} session={session} onSelect={(id) => { const question = subject.questions.find((item) => item.id === session.questionIds[session.currentIndex]); commit(selectResponse(session, id, dependencies.now(), question?.type === "multiple-choice")); }} onNavigate={(index) => { const next = goToQuestion(session, index, dependencies.now()); if (next !== session) commit(next); }} onSubmit={submit} /> : <TestResults subject={subject} session={session} onRetake={retake} onNew={() => setSession(null)} onLearn={() => { window.location.href = `/subjects/${subject.slug}/study?mode=learn`; }} />}</>;
}
