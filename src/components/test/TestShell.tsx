...
export function TestShell({ subject }: { subject: Subject }) {
  ...
  return <>{notice && <div className="notice" role="status">{notice}</div>}{!session ? <TestSetup total={subject.questions.length} unmastered={unmastered} onStart={start} /> : session.status === "active" ? <TestRunner subject={subject} session={session} onSelect={(id) => commit(selectResponse(session, id, dependencies.now()))} onNavigate={(index) => { const next = goToQuestion(session, index, dependencies.now()); if (next !== session) commit(next); }} onSubmit={submit} /> : <TestResults subject={subject} session={session} onRetake={retake} onNew={() => setSession(null)} onLearn={() => { window.location.href = `/subjects/${subject.slug}/study?mode=learn`; }} />}</>;
}
