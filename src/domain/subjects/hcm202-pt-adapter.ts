...
const rawSchema = z.object({
  ...
  subject: z.object({ code: z.literal("HCM202"), title: z.string().min(1), assessment: z.literal("Progress Test"), language: z.literal("vi") }).passthrough(),
  source: z.object({ files: z.array(z.string().min(1)).min(1), pages: z.number().int().nonnegative(), note: z.string() }).passthrough(),
...
export const hcm202PtRawSchema = z.preprocess((value) => {
...
export function adaptHcm202Pt(value: unknown): Subject {
  const raw = hcm202PtRawSchema.parse(value);
  ...
  return subjectSchema.parse({
    ...
    slug: "hcm202-pt",
    code: "HCM202",
    name: "HCM202 - Tư tưởng Hồ Chí Minh - Progress Test",
    description: `Bộ ${questions.length} câu hỏi Progress Test môn Tư tưởng Hồ Chí Minh.`,
    assessment: "Progress Test",
    language: "vi",
    questionCount: questions.length,
    source: { ...raw.source, file: raw.source.files.join(", "), pageCount: raw.source.pages, note: raw.source.note },
    dataQuality: {
