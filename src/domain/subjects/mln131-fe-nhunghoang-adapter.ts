...
const rawQuestionSchema = z.object({
  ...
  source: z.object({ file: z.string().min(1), sourceQuestionNumber: z.number().int().positive(), lines: z.array(z.number().int().positive()), basis: z.string().min(1).optional() }).passthrough()
...
const rawSchema = z.object({
  ...
  subject: z.object({ code: z.literal("MLN131"), title: z.string().min(1), assessment: z.literal("Final Exam"), collection: z.literal("Nhung Hoang"), language: z.literal("vi") }).passthrough(),
...
export const mln131FeNhunghoangRawSchema = z.preprocess((value) => {
...
export function adaptMln131FeNhunghoang(value: unknown): Subject {
  const raw = mln131FeNhunghoangRawSchema.parse(value);
  const questions = raw.questions.map((question) => ({
    id: question.id, number: question.number, type: typeMap[question.type], question: question.question,
    ...
    correctAnswers: question.correctAnswers, explanation: question.explanation.trim() ? question.explanation : null,
    source: { ...question.source, pages: question.sourcePages }, sourcePages: question.sourcePages,
    answerTextFromSource: question.answerTextFromSource, ...(question.sourceNotes === undefined ? {} : { sourceNotes: question.sourceNotes }),
  ...
  return subjectSchema.parse({ schemaVersion: 1, contentVersion: 1, id: "mln131-fe-nhunghoang", slug: "mln131-fe-nhunghoang", code: "MLN131", name: "MLN131 - Chủ nghĩa xã hội khoa học - Final Exam - Nhung Hoang", description: `Bộ ${questions.length} câu hỏi Final Exam môn Chủ nghĩa xã hội khoa học - Nhung Hoang.`, assessment: "Final Exam", language: "vi", questionCount: questions.length, source: { ...raw.source, file: raw.source.files.join(", "), pageCount: 0, note: raw.source.note, collection: raw.subject.collection }, dataQuality: { needsReviewCount: questions.filter((question) => question.needsReview).length, duplicatePromptGroups, ...(raw.dataQuality.reviewBasis === undefined ? {} : { reviewBasis: raw.dataQuality.reviewBasis }) }, questions });
}
