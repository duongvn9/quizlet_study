...
const ignoredFiles = new Set(["hcm202_fe_nhunghoang.json"]);
...
const subjectOrder: (keyof typeof adapters)[] = ["hcm202_fe_chubedan.json", "hcm202_pt.json", "pmg201c.json", "fe-swd392.json", "mln122.json", "mma301.json", "swd392.json"];
const files = subjectOrder;
const ids = new Set<string>();
const slugs = new Set<string>();
const normalizedFiles = new Set<string>();
const subjects = files.map((file) => {
  if (!["hcm202_fe_chubedan.json", "hcm202_pt.json"].includes(file) && !/^[a-z0-9-]+\.json$/.test(file)) throw new Error(`Invalid subject filename: ${file}`);
  ...
  const expectedSlug = file === "hcm202_pt.json" ? "hcm202-pt" : file === "hcm202_fe_chubedan.json" ? "hcm202-fe-chubedan" : basename(file, ".json");
